...
async function getAIConfig(): Promise<AIProviderConfig> {
  ...
  if (!envConfig.apiKey) {
    throw new Error(
      `No API key found for ${finalProvider}. Please set AI_API_KEY environment variable or create .env file with API key.`
    );
  }
...
function displayTestResults(result: any, resultFile: string) {
  ...
  if (executionResult?.steps?.length > 0) {
    ...
    executionResult.steps.forEach((step: any, index: number) => {
      ...
      console.log(
        `${index + 1}. ${stepIcon} ${chalk[stepColor](step.stepId)} (${step.duration}ms)`
      );
      if (step.error) {
